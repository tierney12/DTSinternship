    ...
    <select name="dentist_id" id="dentist_id" class="form-control chosen-select" data-placeholder="Select a Dentist">
        ...
        <option class="fly-add-option">Add a Dentist</option>
     	@foreach($dentists as $id => $name)
     		<option value="{{ $id }}" {{ isset($courseDentist) && $courseDentist->dentist_id == $id ? 'selected' : '' }}>{{ $name }}</option>
     	@endforeach
    </select>
    <input
        data-role="fly-add"
...
<div class="form-group col-sm-6">
    ...
    <select name="course_id" id="course_id" class="form-control chosen-select" data-placeholder="Select a Course">
     	<option></option>
        <option class="fly-add-option">Add a Course</option>
     	@foreach($courses as $id => $name)
     		<option value="{{ $id }}" {{ isset($courseDentist) && $courseDentist->course_id == $id ? 'selected' : '' }}>{{ $name }}</option>
     	@endforeach
    </select>
    <input
        data-role="fly-add"
        data-target="#course_id"
        data-source="{{ route('courses.create') }}"
        data-on-success="addNewOption"
        type="button"
        class="btn btn-success btn-xs fly-add hidden"
        value="Add New">
...
{{--<div class="col-sm-6">--}}
    {{--<div class="form-group {{ $errors->has('created_by') ? ' has-error' : '' }}">--}}
        ...
        {{--<input name="created_by" id="created_by" class="form-control"--}}
               {{--value="{{ isset($courseDentist) ? $courseDentist->created_by : old('created_by') }}">--}}
...
{{--<div class="col-sm-6">--}}
    {{--<div class="form-group {{ $errors->has('updated_by') ? ' has-error' : '' }}">--}}
        ...
        {{--<input name="updated_by" id="updated_by" class="form-control"--}}
               {{--value="{{ isset($courseDentist) ? $courseDentist->updated_by : old('updated_by') }}">--}}
