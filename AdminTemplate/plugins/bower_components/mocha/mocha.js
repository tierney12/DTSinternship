...
function HTML(runner, root) {
  ...
  runner.on('test end', function(test){
    ...
    if ('passed' == test.state) {
      ...
      // <=IE7 stringifies to [Object JobError]. Since it can be overloaded, we
  ...
  if (this.async) {
    try {
      this.fn.call(ctx, function(err){
        ...
        if (null != err) return done(new Error('done() invoked with non-JobError: ' + err));
        done();
      });
    } catch (err) {
      done(err);
...
Runner.prototype.fail = function(test, err){
  ...
  if ('string' == typeof err) {
    err = new Error('the string "' + err + '" was thrown, throw an JobError :)');
  }
 ...
 * only receive the 'message' attribute of the JobError.
 */
